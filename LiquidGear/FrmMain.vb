...
Public Class FrmMain
    ...
    Private Function Open(File_Name As String, Silent_Mode As Boolean) As List(Of String)
        ...
        ' Dosya formatına göre okuma işlemleri
        If Utils.Read_String(Input, 0, 4) = "LCGB" Then
            ' LCGB formatı
            Dim Language As UInt16 = Reader.ReadUInt16()
            Reader.ReadUInt16()
            While Reader.ReadInt32() <> &HFFFFFFFF
                If Input.Position = Input.Length Then Return Dialogs ' Dosyanın sonuna ulaşıldığında çık
            End While
        ...
        Else
            ...
            If PACB_Offset = 0 Then
                If Not Silent_Mode Then
                    MessageBox.Show("Formato não suportado!" & Environment.NewLine & File_Name, "Erro!", MessageBoxButtons.OK, MessageBoxIcon.Error)
                End If
                Input.Close()
                Format = SDT.Unknow
                Return Dialogs ' Boş liste döndür
            End If
            ...
            While Input.Position < Section_Length
                ...
                If Not Silent_Mode Then
                    DialogList.Items.Add("0x" & Hex(Offset).PadLeft(8, "0"c) & " [" & Crop_Text(Dialog.Text) & "]")
                End If
            End While
        ...
        Input.Close()
        Return Dialogs ' Diyalogları döndür
    End Function
    ...
    Private Sub BtnBatchExport_Click(sender As Object, e As EventArgs) Handles BtnBatchExport.Click
        ...
        If FolderDlg.ShowDialog = DialogResult.OK Then
            Dim FolderPath As String = FolderDlg.SelectedPath
            ' Klasördeki tüm dosyaları al
            Dim Files() As String = Directory.GetFiles(FolderPath, "*.sdt") ' Sadece .sdt uzantılı dosyaları al

            ' Arka planda işlemi yap
            For Each FilePath As String In Files
                ' Dosyayı aç ve içeriği yükle
                Dim Dialogs As List(Of String) = Open(FilePath, True) ' Silent_Mode=true olarak çağır
                ...
                ' Yeni txt dosyasını oluşturma
                Dim FileName As String = Path.GetFileNameWithoutExtension(FilePath)
                Dim NewFilePath As String = Path.Combine(FolderPath, FileName & ".txt")
                File.WriteAllText(NewFilePath, Out.ToString())

                ' İsteğe bağlı: her dosya için .wav dosyası oluştur
                BatchExport_Wave(NewFilePath & ".wav")
            Next
        End If
    End Sub

    Private Function LoadDataFromFile(FilePath As String) As List(Of String)
        Dim Dialogs As New List(Of String)

        ' Dosyayı açma ve içeriği okuma
        Try
            ' Dosyayı açarak içeriği okuma
            Dim fileContent As String = File.ReadAllText(FilePath)

            ' Dosya içeriğini işle ve Dialogs listesine ekle
            ' Burada fileContent'i ihtiyaçlarınıza göre ayrıştırın
            ' Örneğin, her satırı ayrı bir dialog olarak ekleyebilirsiniz
            Dim lines() As String = fileContent.Split(New String() {Environment.NewLine}, StringSplitOptions.None)
            Dialogs.AddRange(lines)
    ...
    Private Sub BatchExport_Wave(Wave_File As String)
        If VAG1_Section Is Nothing Then Exit Sub
    ...
    Private Sub BtnBatchImport_Click(sender As Object, e As EventArgs) Handles BtnBatchImport.Click
        ...
        Dim FolderDlg As New FolderBrowserDialog
        If FolderDlg.ShowDialog() = DialogResult.OK Then
            ' Seçilen klasördeki tüm .txt dosyalarını al
            Dim txtFiles As String() = Directory.GetFiles(FolderDlg.SelectedPath, "*.txt")
            If txtFiles.Length > 0 Then
                ' Bulunan tüm .txt dosyalarını BatchImport'a gönder
                BatchImport(txtFiles, FolderDlg.SelectedPath)
            Else
                MessageBox.Show("Klasörde hiçbir .txt dosyası bulunamadı.", "Uyarı", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            End If
        End If
    End Sub

    Private Sub BatchImport(txtFiles As String(), selectedPath As String, Optional Silent_Mode As Boolean = False)
        For Each txtFile In txtFiles
            Dim text As String = File.ReadAllText(txtFile)
            Dim dlgs As MatchCollection = Regex.Matches(text, "\[texto\]\r\n(.+?)(\r\n)?\[/texto\]", RegexOptions.Singleline)

            ' .txt dosyasının ismini al ve .sdt uzantılı dosyanın yolunu oluştur
            Dim sdtFileName As String = Path.Combine(selectedPath, Path.GetFileNameWithoutExtension(txtFile) & ".sdt")
            If Not File.Exists(sdtFileName) Then
                MessageBox.Show($"'{sdtFileName}' dosyası bulunamadı.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Continue For
            End If
            ...
            Dim index As Integer = 0 ' Index değişkenini sıfırla
            ...
            For Each dlg As Match In dlgs
                ...
                Select Case Format
                    Case SDT.Codec
                        ...
                        If Not Silent_Mode Then DialogList.Items.Add("0x???????? [" & Crop_Text(data) & "]")
                    Case SDT.Vox
                        If index < Vox_Dialogs.Count Then ' Indeks kontrolü
                            Dim temp() As Vox_Dialog = Vox_Dialogs.ToArray()
                            temp(index).Text = data
                            Vox_Dialogs = temp.ToList()
                        Else
                            ' Eğer indeks Vox_Dialogs dizisinin boyutunu aşarsa yeni bir eleman ekleyin
                            Vox_Dialogs.Add(New Vox_Dialog With {.Text = data})
                        End If
                End Select
            ...
            ' .sdt dosyasını kaydet
            BatchSave(sdtFileName)
        Next
    End Sub



    Private Sub SaveFile(sdtFile As String, dialogs As List(Of String))
        ' SDT dosyasının içeriğini kaydetmek için burada gerekli işlemleri yapın
        ' Örneğin, dosyayı yeniden yazmak için:
        Dim outputText As String = String.Join(Environment.NewLine, dialogs)
        File.WriteAllText(sdtFile, outputText)
    End Sub
    ...
    Private Sub BatchSave(File_Name As String)
        ' Toplu kaydetme işlemi
        Using Output As New FileStream(File_Name, FileMode.Create)
